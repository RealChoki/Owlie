<template>
  <div
    ...
    aria-expanded="open"
  >
    <div class="selected-option selected-option-active">{{ selectedOptionLabel }}</div>
    <font-awesome-icon :icon="open ? ['fas', 'chevron-up'] : ['fas', 'chevron-down']" class="chevron-icon selected-option-active" />
...
.custom-select:hover .selected-option-active {
  opacity: 1;
}

.custom-select-active .selected-option-active {
  opacity: 1;
}
